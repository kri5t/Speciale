...
\item Experiment two showed a slight increase (1,3\%) in accuracy when applying matrix on time of day. Trimming makes less sense in a prediction simulation over an entire since no completely unpredictable values exist in the wind power production training set. It was concluded that trimming experiments should be conducted because it can make sense in individual predictions where trimming can help approach by trimming away outer values.
...
\item The fifth experiment regarding step-ahead forecasting illustrated how the error can be elevated due to how subsequent steps depend on the prediction from its predecessor.
\end{enumerate}

\begin{center}
\begin{longtable}{|c|c|c|c|c|}
\hline
\textbf{Best input combi} & \textbf{Best input with matrix} & \textbf{Best volatility} & \textbf{All time best} & \textbf{Overall improvement} \\
\hline
\endfirsthead
\multicolumn{5}{c}%
{\tablename\ \thetable\ -- \textit{Continued from previous page}} \\
