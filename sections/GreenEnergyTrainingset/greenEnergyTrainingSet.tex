...
This subsection describes the experiments that uncovers the best combination of hidden layers, neurons and epochs as well as the different strategies. Based on the above analysis of wind production influences the network will be build upon following core input parameters; 1) Wind speed; 2) Air Density; and 3) Time of day. Experiments are needed to investigate the impact the influence of wind direction and statistical inputs as presented in section~\ref{sec:usingStatisticalInput}. Furthermore it is needed to test the use of meteorological factors as a substitute for consumption.
...
%heading
\hline                  % inserts single horizontal line
ANN With Meteorological & 0 & 0 & 0 \\ % inserting body of the table
ANN With Consumption & 0 & 0 & 0 \\ [1ex] % [1ex] adds vertical space
\hline %inserts single line
\end{tabular}
...
ANN Without Wind Direction & 0 & 0 & 0 \\ [1ex] % [1ex] adds vertical space
\hline %inserts single line
\end{tabular}
\caption{The table shows the impact of using Wind Direction together with the best consumption approach} % title of Table
\label{table:windDirectionInclusion} % is used to refer this table in the text
\end{table}
