...
Wind & Air &  &  & Wind & Time & & \\ 
Speed & Dens & Consump & Temp & Direction & of Day & MAE & Rank \\
[0.5ex] % inserts table 
%heading
\hline                  % inserts single horizontal line
x & x & & x & x & 0 & 0 & 0 \\ % inserting body of the table
x & x & x & x &  & 0 & 0 & 0 \\ [1ex] % [1ex] adds vertical space
\hline %inserts single line
\end{tabular}
...
Since the co-relation between wind production and wind speed is 0.94 it has been tested on its own. The result is bla and shows that even though the co-relation is close to 1, there is still need for further parameters --- as expected. Air density is added because it is directly proportional to the wind speed as described in~\ref{sec:airDensity}. This brings us at.
As a simple test for the importance of the current wind production development (~\ref{sec:windProductionDev}) the network has been supplied with the wind production from one hour ago. This shows a huge improvement.
